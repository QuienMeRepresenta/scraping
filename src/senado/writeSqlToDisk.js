...
async function writeSqlToDisk(diputados, sqlFilePath) {
  ...
  const sql = diputados
    .map((diputado) => {
      const { nombre, nombreEntidad, desc, imgUrl, link, numeroEntidad, legislatura } = diputado
      const puesto = `Senaduría por el estado de ${nombreEntidad} ${desc} en legislatura ${legislatura}`
