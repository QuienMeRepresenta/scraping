...
async function writeSqlToDisk(diputados, sqlFilePath) {
  const sql = diputados
    .map((d) => {
      const puesto = `Diputación Local Sinaloa por Distrito ${d.distritoLocal}`
...
insert into diputacion_local(actor_politico_id, periodo, distrito_local, estado, link) select id, '[2018-10-01,2021-12-01)'::daterange, ${d.distritoLocal}, 25, '${d.link}' from actores_politicos where nombre_formatted = lower(unaccent('${d.nombre}')) and puesto = '${puesto}' ON CONFLICT  DO NOTHING;`
    })
    .join('\n')
